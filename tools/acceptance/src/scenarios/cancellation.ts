...
export const CANCELLATION_SCENARIOS: readonly Scenario[] = [
  {
    ...
    steps: [
      { action: 'record', name: 'before' },
      {
        ...
        args: ['build'],
        signal: 'SIGINT',
        // Once parsing has reported progress at least once, the stage is genuinely running
        // on every machine. A fixed delay cannot promise that.
        afterOutput: 'Parsing\\s+[1-9][\\d,]*/',
        afterMs: 250,
        expect: {
          exitCode: 1,
...

  {
    ...
    steps: [
      { action: 'record', name: 'before' },
      {
        ...
        args: ['build'],
        signal: 'SIGINT',
        // The first stage that produces output, so the process has certainly started and
        // installed its handlers. Killing a program still loading its modules would prove
        // nothing about cancellation.
        afterOutput: 'Fingerprinting',
        afterMs: 50,
