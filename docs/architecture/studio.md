...
Every image below is generated by `pnpm docs:capture` from a real build, never taken by hand.
Regenerate after any change to a route, and review the diff: a screenshot is a claim about the
interface, and one nobody can retake stops being true on the first edit.

The Playground, which is the route the product's honesty is most visible on. Selections and
omissions carry the same weight, every passage carries its provenance, and the orientation
passages the model also receives are on screen rather than implied by a reserve figure.

![Studio Context Playground](../images/studio-playground.png)

Sources, on the indexed view: one row height, columns that align, and nothing per row that does
not earn its place. At the 2,500-artifact envelope a row carrying an icon, a badge, a chevron
and a menu stops being a list.

![Studio Sources, the indexed artifacts](../images/studio-sources.png)

And the excluded view. Two different decisions are shown as two tables because they happen at
different moments: a rule removes a path before anything is read, and a parser gives up after.
The count on the switch is the sum of both.

![Studio Sources, showing what was excluded and why](../images/studio-excluded.png)

Versions, where every action that changes anything shows its plan first.
...
| 2026-08-04 | Phase 4 closing pass | Chromium 151.0.7922.34 | Every route driven by hand against a live `lore dev` on a five-document project, reading the accessibility tree rather than the pixels. Four defects found that all the automated suites passed over: #199, #200, #202 and #203. All four fixed, each with the assertion that would have caught it. Screen reader and reduced motion remain unsigned. |

#### What the hand pass found that automation did not

Worth keeping, because the pattern repeated four times in one afternoon and will repeat again.

Every criterion in the Phase 4 gate asserted a property of **one artifact**, and every defect
lived in the **disagreement between two representations of the same thing**:

| Defect | What each side said |
|---|---|
| #199 | The bundle cited fifteen passages; the page showed nine |
| #200 | The build id followed an activation; the freshness annotation travelling with it did not |
| #202 | The control counted warnings; the label said exclusions |
| #203 | The screen showed two words; the accessibility tree had one |

None of these is visible to a test that renders a component and looks for a string, because in
every case the string was there. The new tests are written as **equalities** for that reason:
citations on screen equal citations in the bundle, the count on the control equals the rows
rendered, `textContent` equals the full expected sentence including its separator. An equality
fails when a new section, a new count or a new adjacent value is added and forgotten, which a
per-thing assertion never can.

The second lesson is about fixtures. Three of the four had a fixture that agreed with the
defect: `overview: []` and `citations: []` in the Playground fixture, and an excluded fixture
holding only the kind the route rendered. **A fixture written by whoever wrote the component
cannot discover what the component forgot.** That is what the browser suite against a real
server is for, and it is why the new assertions there compare the page to what the API
actually returned rather than to a constant.
