...
| Evidence | Result |
|---|---|
| `main` revision | `1a72274` |
| Local `mise exec -- pnpm verify` | 1,999 passed, 2 skipped |
| Main CI | Run `34292742340`, passed on `1a72274` |
| Phase gates | Run `34292742375`, all eight phase jobs passed |
...
| P1, lifecycle vertical slice | Discovery, ignore rules, path safety, parsers, build identity, object storage, sealing, activation, rollback and CLI verbs | `packages/compiler/test`, `packages/parsers/test`, `packages/backend-local/test`, `packages/cli/test`, acceptance scenarios and phase 1 gate | Baseline covered; review malformed input and interrupted builds |
...
| P3, two-command local product | `lore dev`, watcher behavior, configuration resolution, client connection plans and local serving | `packages/cli/test`, `packages/connect-clients/test`, acceptance scenarios and phase 3 gate | Baseline covered; review process shutdown and editor-save races |
...
| P5, mixed artifacts and tables | HTML, PDF, DOCX, CSV and XLSX parsing, tables, read-only SQL, rules, ranking and client adapters | `packages/parsers/test`, `packages/backend-local/test`, `packages/runtime/test`, `packages/connect-clients/test`, security tests, acceptance scenarios and phase 5 gate | Baseline covered; review malformed and oversized artifact behavior |
