...
describe('projection migrations, issue 87', () => {
  it('create the checked-in D1 projection schema and seed the active pointer row', async () => {
    ...
    expect(result).toEqual({ applied: ['0001', '0002', '0003', '0004'], alreadyApplied: [] });
  ...
  it('are idempotent and record the checked-in checksum once', async () => {
    ...
    expect(second).toEqual({ applied: [], alreadyApplied: ['0001', '0002', '0003', '0004'] });
    ...
    expect(rows).toEqual([
      ...
      {
        id: '0004',
        checksum: PROJECTION_MIGRATIONS[3]?.checksum ?? '',
      },
    ]);
