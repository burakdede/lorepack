...
describe('exclusions', () => {
  ...
  it('distinguishes a format planned for a later release from an unknown one', async () => {
    await withProject({ 'report.pdf': '%PDF', 'photo.png': 'x' }, (root) => {
      const warnings = discoverIn(root).warnings;
      expect(warnings.find((w) => w.path === 'report.pdf')?.code).toBe('planned-format');
      expect(warnings.find((w) => w.path === 'report.pdf')?.message).toContain('later release');
