...
function entries(zip) {
  const found = new Map();
  return new Promise((res, rej) => {
    zip.on('entry', (en) => { found.set(en.fileName, en); zip.readEntry(); });
...
async function parseXml(zip, entry, handlers) {
  ...
  parser.on('doctype', () => { throw new Error('DTD refused'); });
  ...
  await new Promise((res, rej) => {
    parser.on('end', res); parser.on('error', rej); rs.on('error', rej);
    rs.pipe(parser);
  });
}
...
if (found.has('xl/sharedStrings.xml')) {
  ...
  await parseXml(zip, found.get('xl/sharedStrings.xml'), {
    open: (n) => { if (n.name === 'si') { inSi = true; buf = ''; } },
    text: (t) => { if (inSi) buf += t; },
    close: (name) => { if (name === 'si') { shared.push(buf); inSi = false; } },
  });
}
...
if (found.has('xl/workbook.xml')) {
  await parseXml(zip, found.get('xl/workbook.xml'), {
    open: (n) => { if (n.name === 'sheet') sheetNames.push(String(n.attributes.name)); },
  });
}

const sheetEntries = [...found.keys()].filter((k) => /^xl\/worksheets\/sheet\d+\.xml$/.test(k)).sort();
const target = sheetWanted
  ? sheetEntries[sheetNames.indexOf(sheetWanted)] ?? sheetEntries[0]
  : sheetEntries[0];

let rows = 0, cells = 0, formulas = 0;
const merges = [];
const sample = [];
let cur = null, inV = false, inF = false, vbuf = '', fbuf = '';

await parseXml(zip, found.get(target), {
  open: (n) => {
    if (n.name === 'row') rows += 1;
    else if (n.name === 'c') { cur = { ref: n.attributes.r, t: n.attributes.t, s: n.attributes.s }; vbuf = ''; fbuf = ''; }
    else if (n.name === 'v') inV = true;
    else if (n.name === 'f') inF = true;
    else if (n.name === 'mergeCell') merges.push(String(n.attributes.ref));
  },
  text: (t) => { if (inV) vbuf += t; else if (inF) fbuf += t; },
  close: (name) => {
    if (name === 'v') inV = false;
      ...
      if (sample.length < 12) {
        let value, kind;
        if (cur.t === 's') { value = shared[Number(vbuf)]; kind = 'string'; }
        else if (cur.t === 'b') { value = vbuf === '1'; kind = 'boolean'; }
        else if (cur.t === 'inlineStr' || cur.t === 'str') { value = vbuf; kind = 'string'; }
        else if (vbuf === '') { value = null; kind = 'empty'; }
        else if (isDateStyle(cur.s)) {
          // Excel serial, 1900 system, with the historical leap-year bug accounted for.
          value = new Date(Date.UTC(1899, 11, 30) + Number(vbuf) * 86400000).toISOString().slice(0, 10);
          kind = 'date';
        } else { value = Number(vbuf); kind = 'number'; }
...
console.log(JSON.stringify({
  ...
  file: path, sheet: sheetWanted ?? sheetNames[0], sheetNames,
  rows, cells, formulas, merges,
